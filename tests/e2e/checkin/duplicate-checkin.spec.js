...
test.describe('중복 체크인 방지', () => {
  ...
  test('같은 참석자 중복 체크인 방지', async ({ page }) => {
    ...
    // 첫 번째 체크인
    const firstResult = await performQRCheckin(page, 'CHECKIN:REG001', '3001');
    ...
    // 두 번째 체크인 시도
    const secondResult = await performQRCheckin(page, 'CHECKIN:REG001', '3001');
  ...
  test('체크인 시간 기록 확인', async ({ page }) => {
    ...
    const result = await performQRCheckin(page, 'CHECKIN:REG002', '3001');
  ...
  test('체크인 통계 업데이트', async ({ page }) => {
    ...
    // 초기 통계 가져오기
    const initialStats = await getStats(page, '3001');
    
    // 새로운 체크인 수행
    const checkinResult = await performQRCheckin(page, 'CHECKIN:REG004', '3001');
    ...
    // 통계 다시 가져오기
    await page.waitForTimeout(1000);
    const updatedStats = await getStats(page, '3001');
  ...
  test('이벤트간 체크인 독립성', async ({ page }) => {
    // 이벤트 1에서 체크인
    await selectBackendAndLoadData(page, '3001', 'index');
    const event1CheckinResult = await performQRCheckin(page, 'CHECKIN:REG005', '3001');
    ...
    const event2CheckinResult = await performQRCheckin(page, 'CHECKIN:STU001', '3002');
    ...
    await selectBackendAndLoadData(page, '3001', 'index');
    const stats1 = await getStats(page, '3001');
    
    await selectBackendAndLoadData(page, '3002', 'index');
    const stats2 = await getStats(page, '3002');
