...
const SCAN_COOLDOWN = FrontendConfig.scanner.cooldown;
...
async function processCheckin(qrData) {
    ...
    try {
        ...
        // api 객체가 정의되어 있는지 확인
        if (typeof api === 'undefined' || !api || !api.verifyCheckin) {
            console.error('[API 오류] api 객체가 정의되지 않았습니다');
            throw new Error('API 초기화 오류');
        }
        ...
        if (result.success) {
            ...
            if (typeof audioFeedback !== 'undefined' && audioFeedback) {
        ...
        const message = error.message === 'API 초기화 오류' ? '✗ API 초기화 오류' : '✗ 네트워크 오류';
...
async function startScanning() {
    try {
        ...
        let config = isFullscreen && fullscreenConfig ? fullscreenConfig : {
            fps: FrontendConfig.scanner.fps,
            qrbox: FrontendConfig.scanner.qrbox,  // 설정에서 가져옴
            aspectRatio: FrontendConfig.scanner.aspectRatio,  // 설정에서 가져옴
            videoConstraints: {
...
async function handleFullscreenChange() {
    ...
    if (isFullscreen) {
        ...
        fullscreenConfig = {
            fps: isIPhoneCamera ? 5 : FrontendConfig.scanner.fps, // iPhone은 매우 낮은 fps
            qrbox: { 
                width: scanSize, 
                height: scanSize 
            },
